...
class AmazonOrderItemJob < ApplicationJob
  ...
  def update_channel_order(result, channel_order_id)
    result[:body]['payload']['OrderItems'].each do |item|
      ...
      channel_item.channel_product_id = ChannelProduct.find_by(listing_id: channel_item.line_item_id, item_sku: channel_item.sku, channel_type: 'amazon')&.id
      channel_item.save
    end
  end
  ...
  def update_available_stock(item, product, inventory_balance, ordered)
    ...
    product.update(change_log: "API, #{item.channel_product.item_sku}, #{item.channel_order.order_id}, Order Paid,
      #{item.channel_product.listing_id}, #{unshipped}, #{product.inventory_balance}, #{channel_type}",
      unshipped: unshipped, unshipped_orders: product.unshipped_orders.to_i + 1, inventory_balance: inventory_balance)
