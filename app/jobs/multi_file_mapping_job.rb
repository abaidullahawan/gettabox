...
class MultiFileMappingJob < ApplicationJob
  ...
  def case_sensitivity(csv, attribute_data, matchable_data, filename1, filename2)
    ...
    unmatch_data_for_one = FileOne.where(filename: filename1, "#{column_name1}": unmatch_data_for_one).order(id: :asc)
    unmatch_data_for_two = FileTwo.where(filename: filename2, "#{column_name2}": unmatch_data_for_two).order(id: :asc)
